...
client.on("interactionCreate", async (i) => {
  try {
    ...
    const safeReply = async (msg, opt = {}) => {
      if (i.replied || i.deferred) return i.followUp({ content: msg, ...opt });
      return i.reply({ content: msg, ...opt });
    };

    // DEV
    if (i.customId === "dev_heal" && isDev(i.user.id)) {
      p.hp = 1200;
      return safeReply("DEV HEAL");
    }

    if (i.customId === "dev_xp" && isDev(i.user.id)) {
      p.xp += 1000;
      return safeReply("XP +1000");
    }

    if (i.customId === "dev_kill" && isDev(i.user.id)) {
      if (battle) battle.enemyHp = 0;
      return safeReply("ENEMY DEAD");
    }

    if (!battle) return safeReply("전투 없음", { ephemeral: true });
    ...
    if (i.customId === "attack") {
      ...
      battle.enemyHp -= d;
      return safeReply(`⚔️ -${Math.floor(d)}`);
    }
    ...
    if (i.customId === "skill") {
      ...
      return safeReply(`🌀 ${skill} -${d}`);
    }

    if (i.customId === "domain") {
      if (!c.domain) return safeReply("❌ 없음");

      const power = p.hp + p.skillMastery["돌진"] * 10;
      battle.enemyHp -= 400 + power * 0.2;

      return safeReply(`🌌 ${c.domain}`);
    }

    if (i.customId === "reverse") {
      const heal = 100 * p.reverseOutput;
      p.hp = Math.min(1200, p.hp + heal);
      p.reverseOutput += 0.2;

      return safeReply(`♻ +${Math.floor(heal)} (출력 ${p.reverseOutput.toFixed(1)})`);
    }

    if (i.customId === "run") {
      if (Math.random() < 0.7) {
        delete DB.battles[i.user.id];
        return safeReply("🏃 도주 성공");
      }
      return safeReply("❌ 실패");
    }

    if (i.customId === "devpanel" && isDev(i.user.id)) {
      return safeReply("DEV OPENED", { components: devUI(), ephemeral: true });
    }
