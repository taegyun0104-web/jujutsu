...
// ════════════════════════════════════════════════════════
const gifCharacters = {
  이타도리: { color:"#f97316", glow:"#fb923c", title:"Vessel", desc:"주인공, 수쿠나의 그릇" },
  고죠: { color:"#60a5fa", glow:"#3b82f6", title:"The Strongest", desc:"최강의 주술사, 무한" },
  메구미: { color:"#1e293b", glow:"#334155", title:"Ten Shadows", desc:"십종 그림자, 후시구로" },
  노바라: { color:"#ec4899", glow:"#db2777", title:"Straw Doll", desc:"인형술사, 쿠기사키" },
  나나미: { color:"#facc15", glow:"#eab308", title:"Salaryman Sorcerer", desc:"전 샐러리맨 주술사" },
  스쿠나: { color:"#ef4444", glow:"#dc2626", title:"King of Curses", desc:"저주의 왕, 료멘" },
  게토: { color:"#a855f7", glow:"#9333ea", title:"Curse Manipulator", desc:"저주 조작, 게토 스구루" },
  마키: { color:"#84cc16", glow:"#65a30d", title:"Heavenly Restriction", desc:"천여, 제니인 마키" },
  판다: { color:"#22c55e", glow:"#16a34a", title:"Cursed Corpse", desc:"저주 시체, 판다" },
  이누마키: { color:"#38bdf8", glow:"#0ea5e9", title:"Cursed Speech", desc:"저주언어, 이누마키" },
  유타: { color:"#d1d5db", glow:"#9ca3af", title:"Special Grade", desc:"특급 주술사, 옷코츠" },
  히구루마: { color:"#f43f5e", glow:"#e11d48", title:"Judgement Sorcerer", desc:"심판관, 히구루마" },
  죠고: { color:"#fb7185", glow:"#ef4444", title:"Volcano Curse", desc:"화산 저주, 죠고" },
  다곤: { color:"#06b6d4", glow:"#0891b2", title:"Ocean Curse", desc:"바다 저주, 다곤" },
  하나미: { color:"#10b981", glow:"#059669", title:"Nature Curse", desc:"자연 저주, 하나미" },
  마히토: { color:"#a78bfa", glow:"#8b5cf6", title:"Soul Manipulator", desc:"영혼 조작, 마히토" },
  토도: { color:"#f59e0b", glow:"#d97706", title:"Boogie Woogie", desc:"박수, 토도 아오이" },
  하카리: { color:"#fde047", glow:"#facc15", title:"Gambler Domain", desc:"복권 술식, 하카리 키리토" }
};

// GIF 프로필용 유저 데이터
const gifUserData = new Map();

function setGifCharacter(id, name){
  gifUserData.set(id, name);
}

async function renderGIF(user, char){
  const w = 900, h = 350;
...
client.on("messageCreate", async (message) => {
  ...
  if (cmd === "캐릭터선택") {
    const name = args[1];
    if (!name) {
      return message.reply("사용법: !캐릭터선택 [캐릭터이름]\n예시: !캐릭터선택 고죠");
    }
    if (!gifCharacters[name]) {
      return message.reply(`❌ 존재하지 않는 캐릭터입니다.\n\n📖 사용 가능한 캐릭터: ${Object.keys(gifCharacters).join(", ")}\n\n💡 !도감 으로 상세 정보를 확인하세요.`);
    }
    setGifCharacter(userId, name);
    return message.reply(`✨ **${name}** (${gifCharacters[name].title}) 캐릭터가 장착되었습니다!\n📖 설명: ${gifCharacters[name].desc}`);
  }

  if (cmd === "프로필") {
    const selected = gifUserData.get(userId);
    if (!selected) {
      return message.reply("⚠️ 먼저 캐릭터를 선택해주세요!\n사용법: !캐릭터선택 [이름]");
    }
    try {
      const img = await renderGIF(message.author, gifCharacters[selected]);
      return message.reply({ 
        content: `🎴 **${message.author.username}** 님의 프로필\n⚔️ 장착 캐릭터: **${selected}** (${gifCharacters[selected].title})`,
        files: [img] 
      });
    } catch(error) {
      console.error(error);
      return message.reply("❌ 프로필 생성 중 오류가 발생했습니다.");
    }
  }
  ...
  if (cmd === "도감") {
    let desc = "";
    let count = 0;
    for(const [name, data] of Object.entries(gifCharacters)){
      count++;
      desc += `**${count}. ${name}** - ${data.title}\n📖 ${data.desc}\n🎨 컬러: ${data.color}\n\n`;
    }
    const embed = new EmbedBuilder()
      .setTitle("📚 주술회전 캐릭터 도감")
      .setColor(0xff6b6b)
      .setDescription(desc)
      .setFooter({ text: `총 ${Object.keys(gifCharacters).length}명의 캐릭터 | !캐릭터선택 [이름] 으로 장착` })
      .setTimestamp();
  ...
  if (cmd === "dev") {
    const devInfo = new EmbedBuilder()
      .setTitle("👨‍💻 봇 개발자 정보")
      .setColor(0x00ff00)
      .setDescription("**주술회전 통합 봇**\n📌 버전: 3.0.0\n🎨 기능: GIF 프로필 + RPG 시스템")
      .addFields(
        { name: "📋 명령어 목록", value: "`!캐릭터선택` - GIF 프로필 캐릭터 장착\n`!프로필` - GIF 프로필 출력\n`!도감` - 캐릭터 정보 보기\n`!dev` - 개발자 정보\n\n`/전투` `/컬링` `/사멸회유` `/가챠` `/출석` 등 RPG 명령어", inline: false },
        { name: "⚙️ 기술 스택", value: "Discord.js v14\n@napi-rs/canvas\ngif-encoder-2\nPostgreSQL", inline: true },
        { name: "🎮 캐릭터 수", value: `${Object.keys(gifCharacters).length}명 (GIF)\n${Object.keys(CHARACTERS).length}명 (RPG)`, inline: true },
        { name: "💡 팁", value: "캐릭터 이름으로 바로 장착 가능!\n예: `!캐릭터선택 고죠`", inline: false }
      )
      .setFooter({ text: "Made with ❤️ for Jujutsu Kaisen fans" })
      .setTimestamp();
    return message.reply({ embeds: [devInfo] });
  }
  ...
  if (cmd === "전투") {
    ...
    const stats = getPlayerStats(player);
    player.hp = stats.maxHp;
    await message.reply(`✅ 활성 캐릭터를 **${CHARACTERS[charId].name}**(으)로 변경했습니다!`);
    savePlayer(userId);
  }
  else if (cmd === "출석") {
    const now = Date.now();
    const last = player.lastDaily || 0;
    if (now - last < 86400000) return message.reply("⏰ 이미 출석했습니다!");
    const streakBonus = Math.min(player.dailyStreak || 0, 30);
    const totalCrystals = 100 + streakBonus * 5;
    player.crystals += totalCrystals;
    player.lastDaily = now;
    player.dailyStreak = (player.dailyStreak || 0) + 1;
    await message.reply(`✅ 출석! +${totalCrystals}💎 (연속 ${player.dailyStreak}일)`);
    savePlayer(userId);
  }
  else if (cmd === "회복") {
    ...
    player.potion--;
    await message.reply(`✅ 회복! (남은 회복약: ${player.potion})`);
    savePlayer(userId);
  }
  else if (cmd === "코가네가챠") {
    ...
    if (!target) return message.reply("❌ @멘션으로 대상을 지정하세요!");
    ...
    const embed = new EmbedBuilder()
      .setTitle("⚔️ PvP 결투 신청")
      .setDescription(`${target}님, ${message.author}님이 결투를 신청했습니다!`)
      .setColor(0xF5C842);
    const buttons = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`pvp_challenge_accept_${userId}`).setLabel("✅ 수락").setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`pvp_challenge_decline_${userId}`).setLabel("❌ 거절").setStyle(ButtonStyle.Danger)
    ...
    const partyId = `${_partyIdSeq++}`;
    parties[partyId] = { id: partyId, leader: userId, members: [userId], bestWave: 0 };
    await message.reply(`✅ 파티 생성됨! ID: ${partyId}\n!파티초대 @유저`);
  }
  else if (cmd === "파티초대") {
    const target = message.mentions.users.first();
    ...
    if (party.members.length >= 4) return message.reply("❌ 파티 가득 참!");
    if (getPartyId(target.id)) return message.reply("❌ 상대방이 이미 파티 있음!");
    partyInvites[target.id] = { partyId: party.id, inviter: userId };
    const embed = new EmbedBuilder()
      .setTitle("👥 파티 초대")
      .setDescription(`${target}님, ${message.author}님이 파티에 초대했습니다!`)
      .setColor(0x4ade80);
    const buttons = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`party_invite_accept_${party.id}_${target.id}`).setLabel("✅ 수락").setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`party_invite_decline_${party.id}_${target.id}`).setLabel("❌ 거절").setStyle(ButtonStyle.Danger)
    ...
    for (const uid of party.members) {
      if (players[uid]?.hp <= 0) return message.reply(`❌ ${players[uid].name}님이 전투 불능!`);
    }
    const firstEnemy = pickCullingEnemy(1);
    ...
    if (!code) return message.reply("❌ 코드 입력!");
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription([
        ...
        "**🎲 시스템**",
        "`!프로필` `!가챠 1/10` `!코가네가챠` `!활성` `!술식` `!출석` `!회복` `!손가락` `!코드`",
      ].join("\n"))
      .setColor(0xF5C842);
