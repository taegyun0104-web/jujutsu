...
// 파트1 끝 — 파트2로 이어짐
// ════════════════════════════════════════════════════════
// 파트 2/2 — 전투/컬링/사멸회유/PvP/레이드/명령어 핸들러
// 파트1 코드와 합쳐서 하나의 index.js로 사용하세요
// ════════════════════════════════════════════════════════
...
async function handleBattleAction(interaction, player, battle, action) {
  ...
  if (action === "b_reverse") {
    ...
    player.hp = Math.min(stats.maxHp, (player.hp||0) + healAmount);
  ...
  // 행동 불가 체크
  const cannotAct = (player.statusEffects||[]).some(s => s.id==="freeze" || s.id==="stun");
  if (cannotAct) {
    player.statusEffects = (player.statusEffects||[]).map(s => {
      if (s.id==="freeze"||s.id==="stun") return {...s, turns:Math.max(0,s.turns-1)};
      return s;
    }).filter(s => s.turns > 0);
    ...
    if ((player.hp||0) <= 0) {
      ...
      delete battles[interaction.user.id];
      savePlayer(interaction.user.id);
      return interaction.update({ embeds:[new EmbedBuilder().setTitle("⚠️ 행동 불가!").setDescription(log.join("\n")), new EmbedBuilder().setTitle("💀 패배...").setColor(0xe63946).setDescription("> `!회복` 으로 HP를 회복하세요.")], components:[] });
    }
    tickCooldowns(player);
    savePlayer(interaction.user.id);
    const embed = new EmbedBuilder().setTitle("⚠️ 행동 불가!").setColor(0x888888).setDescription(log.join("\n"))
      .addFields({name:"내 HP",value:`💚 **${player.hp}** / **${stats.maxHp}**\n상태: ${statusStr(player.statusEffects)}`,inline:true},{name:`${enemy.emoji} ${enemy.name}`,value:`💚 **${enemy.currentHp}** / **${enemy.hp}**`,inline:true});
    return interaction.update({ embeds:[embed], components:[mkBattleButtons(player)] });
  }

  let dmg = 0, isBlack = false, isCrit = false, skillUsed = "";
  ...
  if (action === "b_attack") {
    ...
    if (!hit) { log.push("⚡ 공격이 **빗나갔다!**"); }
      ...
      if (isBlack) { dmg = Math.floor(dmg*2.5); player.crystals = (player.crystals||0)+50; log.push(getBlackFlashArt()); log.push(`💥 **흑섬 발동!!** **${dmg}** 피해! (×2.5) +50💎`); }
    ...
    if (!hit) { log.push("⚡ 술식이 **빗나갔다!**"); }
      ...
      if (isBlack) { dmg = Math.floor(dmg*2.5); player.crystals = (player.crystals||0)+50; log.push(`⚫ **흑섬!!** **${dmg}** 피해! (×2.5) +50💎`); }
      else if (isCrit) log.push(`✨ **치명타!** **${dmg}** 피해! (×1.5)`);
    ...
    if (!hit) { log.push("⚡ 주력 스킬이 **빗나갔다!**"); }
      ...
      if (isBlack) { dmg = Math.floor(dmg*2.5); player.crystals = (player.crystals||0)+50; log.push(`⚫ **흑섬!!** **${dmg}** 피해! (×2.5)`); }
      else if (isCrit) log.push(`✨ **치명타!** **${dmg}** 피해! (×1.5)`);
  ...
  const embed = new EmbedBuilder()
    .setTitle(isBlack ? "⚫ 흑 섬 ⚫" : action==="b_attack" ? "⚔️ 공격!" : action==="b_main" ? "⭐ 주력 술식!" : `🌀 ${skillUsed||"술식"}!`)
    .setColor(isBlack ? 0x0a0a0a : action==="b_attack" ? 0xff6b35 : 0x7c5cfc)
    .setDescription(log.join("\n"))
    .addFields(
      { name:`${CHARACTERS[player.active].emoji} 내 HP`, value:`💚 **${Math.max(0,player.hp||0)}** / **${stats.maxHp}**`, inline:true },
      { name:`${enemy.emoji} ${enemy.name}`, value:`💚 **${enemy.currentHp}** / **${enemy.hp}**`, inline:true }
    );
  ...
  embed.spliceFields(0, 2,
    { name:"내 HP", value:`💚 **${Math.max(0,player.hp||0)}** / **${stats.maxHp}**\n상태: ${statusStr(player.statusEffects)}`, inline:true },
    { name:`${enemy.name} HP`, value:`💚 **${enemy.currentHp}** / **${enemy.hp}**\n상태: ${statusStr(enemy.statusEffects||[])}`, inline:true }
  );
  tickCooldowns(player);
...
async function handleCullingAction(interaction, player, culling, action) {
  ...
  if (action === "c_reverse") {
    ...
    if ((player.reverseCooldown||0) > 0) return interaction.reply({ content:`❌ 쿨다운: **${player.reverseCooldown}턴**`, ephemeral:true });
    ...
    if (!hit) { log.push("⚡ 공격이 **빗나갔다!**"); }
      ...
      if (isBlack) { dmg = Math.floor(dmg*2.5); player.crystals = (player.crystals||0)+50; log.push(`⚫ **흑섬!** **${dmg}** 피해! +50💎`); }
      else if (isCrit) log.push(`✨ **치명타!** **${dmg}** 피해! (×1.5)`);
      ...
      dmg = res.dmg; isCrit = res.isCrit;
      const isBlack = isBlackFlash();
      if (isBlack) { dmg = Math.floor(dmg*2.5); player.crystals = (player.crystals||0)+50; log.push(getBlackFlashArt()); log.push(`⚫ **흑섬!** **${dmg}** 피해! +50💎`); }
      else if (isCrit) log.push(`✨ **치명타!** **${dmg}** 피해! (×1.5)`);
    ...
    if (culling.enemyHp <= 0) {
      ...
      player.xp = (player.xp||0)+xp; player.crystals = (player.crystals||0)+cr;
      ...
      if ((player.hp||0) <= 0) {
        ...
        delete cullings[interaction.user.id];
        savePlayer(interaction.user.id);
        return interaction.update({ embeds:[new EmbedBuilder().setTitle("💀 컬링 종료!").setColor(0xe63946).setDescription(`> WAVE **${culling.wave}** 에서 쓰러졌습니다!\n> 최고기록: WAVE **${player.cullingBest}**`)], components:[] });
      }
    }
  }
  tickCooldowns(player);
...
async function handleJujutsuAction(interaction, player, jujutsu, action) {
  ...
  if (action === "j_reverse") {
    ...
    if (!hit) { log.push("⚡ 공격이 **빗나갔다!**"); }
      ...
      dmg = res.dmg; isCrit = res.isCrit;
      const isBlack = isBlackFlash();
      if (isBlack) { dmg = Math.floor(dmg*2.5); player.crystals = (player.crystals||0)+50; log.push(`⚫ **흑섬!** **${dmg}** 피해!`); }
      else if (isCrit) log.push(`✨ **치명타!** **${dmg}** 피해! (×1.5)`);
      else log.push(`> ⚔️ 공격! **${dmg}** 피해!`);
    }
    ...
    if (jujutsu.enemyHp <= 0) {
      ...
      if (jujutsu.points >= 15) {
        player.crystals = (player.crystals||0)+300; player.xp = (player.xp||0)+500;
        if (jujutsu.points > (player.jujutsuBest||0)) player.jujutsuBest = jujutsu.points;
        delete jujutsus[interaction.user.id];
        savePlayer(interaction.user.id);
        return interaction.update({ embeds:[new EmbedBuilder().setTitle("🏆 사멸회유 클리어!").setColor(0xF5C842).setDescription(`> 15포인트 달성! **+300💎 +500XP** 보너스!\n> 총 XP: **${jujutsu.totalXp}** | 총 💎: **${jujutsu.totalCrystals}**\n${getNewlyCompletedQuestMsg(player)}`)], components:[] });
      }
...
async function handlePvpAction(interaction, player, session, action) {
  ...
  if (action === "pvp_surrender") {
    player.pvpLosses = (player.pvpLosses||0)+1; opp.pvpWins = (opp.pvpWins||0)+1;
    ...
    return interaction.update({ embeds:[new EmbedBuilder().setTitle(`🏳️ ${player.name} 항복!`).setColor(0xe63946).setDescription(`> **${opp.name}** 의 승리!`)], components:[] });
  }
  ...
  if (action === "pvp_atk") {
    ...
    if ((session[selfKeys.skillCdKey]||0) > 0) return interaction.reply({ content:`❌ 술식 쿨다운 **${session[selfKeys.skillCdKey]}턴** 남음!`, ephemeral:true });
    ...
    if (!hit) { log.push("⚡ 술식이 빗나갔다!"); }
      ...
      else if (res.isCrit) { log.push(`✨ **치명타!** **${dmg}** 피해!`); }
      else { log.push(`> 🌀 **${skill.name}** — **${dmg}** 피해!`); }
    ...
    if (session[`domainUsed${selfKeys.hpKey==="hp1"?"1":"2"}`]) return interaction.reply({ content:"❌ 이미 영역전개를 사용했습니다!", ephemeral:true });
    const dmg = Math.floor(selfStats.atk * 2.8);
    session[oppKeys.hpKey] = Math.max(0, session[oppKeys.hpKey] - dmg);
    session[`domainUsed${selfKeys.hpKey==="hp1"?"1":"2"}`] = true;
    log.push(`🌌 **${ch.domain}** — **${dmg}** 피해!`);
  } else if (action === "pvp_reverse") {
    if (!REVERSE_CHARS.has(player.active)) return interaction.reply({ content:"❌ 반전술식 불가!", ephemeral:true });
    if ((session[selfKeys.reverseCdKey]||0) > 0) return interaction.reply({ content:`❌ 쿨다운 **${session[selfKeys.reverseCdKey]}턴** 남음!`, ephemeral:true });
  ...
  // 상태이상 틱 (자신)
  if ((session[selfKeys.statusKey]||[]).length > 0) {
    let tickDmg = 0;
    for (const se of (session[selfKeys.statusKey]||[])) {
      const def = STATUS_EFFECTS[se.id]; if (!def) continue;
      if (se.id==="poison") { const d=Math.max(1,Math.floor(selfStats.maxHp*0.05)); tickDmg+=d; log.push(`${def.emoji} **${def.name}** — **${d}** 피해!`); }
      if (se.id==="burn")   { const d=Math.max(1,Math.floor(selfStats.maxHp*0.08)); tickDmg+=d; log.push(`${def.emoji} **${def.name}** — **${d}** 피해!`); }
      se.turns--;
    }
  ...
  if (session[oppKeys.hpKey] <= 0) {
    player.pvpWins = (player.pvpWins||0)+1; opp.pvpLosses = (opp.pvpLosses||0)+1;
    updateQuestProgress(player, "pvp_win", 1);
    const sid = Object.keys(pvpSessions).find(k=>pvpSessions[k]===session); if (sid) delete pvpSessions[sid];
    savePlayer(player.id); savePlayer(opp.id);
    return interaction.update({ embeds:[pvpEmbed(session,log), new EmbedBuilder().setTitle(`🏆 ${player.name} 승리!`).setColor(0xF5C842).setDescription(`> **${player.name}** 이 **${opp.name}** 을 격파!`)], components:[] });
  }
  if (session[selfKeys.hpKey] <= 0) {
    ...
    const sid = Object.keys(pvpSessions).find(k=>pvpSessions[k]===session); if (sid) delete pvpSessions[sid];
    savePlayer(player.id); savePlayer(opp.id);
    return interaction.update({ embeds:[pvpEmbed(session,log), new EmbedBuilder().setTitle(`💀 ${player.name} 패배!`).setColor(0xe63946)], components:[] });
  }
...
async function handleRaidAction(interaction, player, raidSession, action) {
  ...
  if (action === "r_retreat") {
    ...
    if (raidSession.members.length === 0) {
      const sid = Object.keys(raidSessions).find(k=>raidSessions[k]===raidSession); if (sid) delete raidSessions[sid];
    }
  ...
  if (cannotAct) {
    ...
    if ((player.hp||0) <= 0) {
      raidSession.members = raidSession.members.filter(id=>id!==player.id);
      if (raidSession.members.length===0) { const sid=Object.keys(raidSessions).find(k=>raidSessions[k]===raidSession); if(sid) delete raidSessions[sid]; }
    }
  ...
  if (action === "r_attack") {
    ...
    if (!hit) { log.push(`> ⚡ **${player.name}**의 공격이 빗나갔다!`); }
      ...
      dmg = res.dmg;
      const isBlack = isBlackFlash();
      if (isBlack) { dmg = Math.floor(dmg*2.5); player.crystals=(player.crystals||0)+50; log.push(getBlackFlashArt()); log.push(`> 💥 **${player.name} 흑섬!** **${dmg}** 피해! +50💎`); }
      else if (res.isCrit) log.push(`> ✨ **${player.name} 치명타!** **${dmg}** 피해! (×1.5)`);
    ...
    if (boss.adaptationSkill && (raidSession.adaptedSkills||[]).includes(skill.name)) {
      ...
      player.skillCooldown = 5; tickCooldowns(player); savePlayer(player.id);
      await doRaidBossAttack(player, raidSession, boss, log);
      return interaction.update({ embeds:[raidEmbed(raidSession,log)], components:[mkRaidButtons(player)] });
    }
    const hit = rollHit(player.statusEffects, []);
    if (!hit) { log.push(`> ⚡ **${skill.name}**이 빗나갔다!`); }
      ...
      const fx = getSkillEffect(skill.name);
      log.push(fx.art); log.push(`> *"${fx.flavorText}"*`);
      if (isBlack) { dmg = Math.floor(dmg*2.5); player.crystals=(player.crystals||0)+50; log.push(`> ⚫ **${player.name} 흑섬!** **${dmg}** 피해! +50💎`); }
      else if (res.isCrit) log.push(`> ✨ **${player.name} 치명타!** **${dmg}** 피해! (×1.5)`);
      else log.push(`> 🌀 **${player.name}**: **${skill.name}** — **${dmg}** 피해!`);
      if (boss.adaptationSkill) {
        if (!raidSession.adaptedSkills) raidSession.adaptedSkills=[];
        if (!raidSession.adaptedSkills.includes(skill.name)) { raidSession.adaptedSkills.push(skill.name); log.push(`> 🔄 **마허라가라**가 **${skill.name}**에 적응 시작! 다음부터 무효!`); }
      }
    }
    player.skillCooldown = 5;
    updateQuestProgress(player, "skill_use", 1);
  ...
  if (raidSession.hp <= 0) {
    ...
    for (const uid of raidSession.members) {
      ...
      p.xp = (p.xp||0)+Math.floor(boss.xp*kb.xp);
      p.crystals = (p.crystals||0)+Math.floor(boss.crystals*kb.crystal);
    ...
    const sid = Object.keys(raidSessions).find(k=>raidSessions[k]===raidSession); if (sid) delete raidSessions[sid];
    return interaction.update({ embeds:[new EmbedBuilder().setTitle("🏆 레이드 클리어!").setColor(0xF5C842).setDescription([`\`\`\`ansi\n\u001b[1;33m║  🏆  RAID CLEAR!!  🏆  ║\n\`\`\``,`> **${boss.name}** 격파!`,`> 보상: +${boss.xp}XP +${boss.crystals}💎 +3🧪`,`> 📦 재료: ${formatDrops(drops)}`].join("\n"))], components:[] });
  }
...
async function handlePartyCullingAction(interaction, player, session, action) {
  ...
  const enemy = session.currentEnemy; const log = [];
  ...
  if (!hit) { log.push(`⚡ **${player.name}**의 공격이 빗나갔다!`); }
    ...
    if ((player.skillCooldown||0) > 0) return interaction.reply({ content:"❌ 술식 쿨다운!", ephemeral:true });
    const skill = getCurrentSkill(player, player.active);
    const res = calcSkillDmgForPlayer(player, skill.dmg); dmg = res.dmg;
    const isBlack = isBlackFlash();
    if (isBlack) { dmg=Math.floor(dmg*2.5); log.push(`⚫ **${player.name}** 흑섬! **${dmg}** 피해!`); }
    else if (res.isCrit) log.push(`✨ **${player.name}** 치명타! **${dmg}** 피해!`);
    else log.push(`> 🌀 **${player.name}**: ${skill.name} — **${dmg}** 피해!`);
    player.skillCooldown = 5;
  } else {
    const res = calcDmgForPlayer(player, enemy.def); dmg = res.dmg;
    const isBlack = isBlackFlash();
    if (isBlack) { dmg=Math.floor(dmg*2.5); log.push(`⚫ **${player.name}** 흑섬! **${dmg}** 피해!`); }
    else if (res.isCrit) log.push(`✨ **${player.name}** 치명타! **${dmg}** 피해!`);
    else log.push(`> ⚔️ **${player.name}** 공격! **${dmg}** 피해!`);
  ...
  if (session.enemyHp <= 0) {
    session.kills++; session.totalXp+=enemy.xp; session.totalCrystals+=enemy.crystals;
    for (const uid of party.members) {
      const p=players[uid]; if (!p) continue;
      p.xp=(p.xp||0)+Math.floor(enemy.xp/party.members.length);
      p.crystals=(p.crystals||0)+Math.floor(enemy.crystals/party.members.length);
      addMaterials(p, rollDrops(enemy.id));
    ...
    const tgt=party.members[Math.floor(Math.random()*party.members.length)]; const p2=players[tgt];
    if (p2) { const eDmg=calcDmg(enemy.atk,getPlayerStats(p2).def); p2.hp=Math.max(0,(p2.hp||0)-eDmg); log.push(`> 💢 **${enemy.name}** → **${p2.name}** **${eDmg}** 피해!`); }
    if (party.members.every(uid=>(players[uid]?.hp||0)<=0)) { delete cullings[party.id]; return interaction.update({ content:"💀 파티 전원 쓰러짐!", embeds:[], components:[] }); }
  }
  tickCooldowns(player); savePlayer(player.id);
...
function createPvpSession(p1Id, p2Id) {
  ...
  return { id:`pvp_${Date.now()}`, p1Id, p2Id, hp1:s1.maxHp, hp2:s2.maxHp, maxHp1:s1.maxHp, maxHp2:s2.maxHp, status1:[], status2:[], skillCd1:0, skillCd2:0, reverseCd1:0, reverseCd2:0, domainUsed1:false, domainUsed2:false, turn:p1Id, round:1 };
}
...
function devPanelEmbed() {
  ...
  return new EmbedBuilder().setColor(0xFF0000).setTitle("🔧 개발자 패널").setDescription("```ansi\n\u001b[1;31m╔══════════════════════════════════╗\n║  🔧  DEV  PANEL  —  ADMIN ONLY  ║\n╚══════════════════════════════════╝\n```")
    .addFields(
      ...
      {name:"🛠️ 개발자 명령어",value:[`> \`!아이템지급 [크리스탈|회복약|손가락|재료명] [수량]\``,`> \`!쿨다운초기화\``,`> \`!전체저장\``,`> \`!플레이어정보 @유저\``,`> \`!개발자패널\``].join("\n"),inline:false}
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "char_select") {
      ...
      player.hp = stats.maxHp;               // HP 완전 회복
      savePlayer(userId);
      const ch = CHARACTERS[charId];
      const ri = GACHA_RARITY[ch.grade]||GACHA_RARITY["3급"];
      return interaction.update({
        content: null,
        embeds: [new EmbedBuilder().setColor(ri.color)
          ...
          .setDescription([
            ...
            `> 🌀 현재 스킬: **${getCurrentSkill(player,charId)?.name||"없음"}**`,
            `> 🌌 영역전개: \`${ch.domain||"없음"}\``,
          ].join("\n"))],
        components: [],
      });
...
async function handleSlashCommand(interaction, commandName, player, userId, user) {
  ...
  if (commandName === "전투") {
    ...
    const stats = getPlayerStats(player);
    const embed = new EmbedBuilder().setTitle(eBase.id==="e_sukuna"?"🔴 료멘 스쿠나 출현!":"⚔️ 전투 시작!").setColor(eBase.id==="e_sukuna"?0x8b0000:0xff0000)
      .setDescription(`**${enemy.emoji} ${enemy.name}** 이(가) 나타났다!\n내 HP: ${player.hp||0}/${stats.maxHp}`)
  ...
  if (commandName === "코가네가챠") {
    if ((player.crystals||0) < 200) return interaction.reply({ content:"💎 부족! (필요: 200)", ephemeral:true });
    ...
    await new Promise(r=>setTimeout(r,1800)); await interaction.editReply({ embeds:[koganeLoadingEmbed(2)] });
    await new Promise(r=>setTimeout(r,1800)); await interaction.editReply({ embeds:[koganeLoadingEmbed(3)] });
    await new Promise(r=>setTimeout(r,1500));
  ...
  if (commandName === "활성") {
    ...
    return interaction.reply({ content:"🎭 **캐릭터를 선택하세요:**", components:[mkCharSelectMenu(player,"char_select")] });
  }

  if (commandName === "도감") {
    const allIds = Object.keys(CHARACTERS);
    const ownedList = player.owned.map(id=>{ const c=CHARACTERS[id]; if(!c)return""; const m=getMastery(player,id); const ri=GACHA_RARITY[c.grade]||GACHA_RARITY["3급"]; return `> ${id===player.active?"▶️**[활성]**":"　"}${c.emoji} **${c.name}** \`[${c.grade}]\` ${ri.stars} 숙련\`${m}\``; }).join("\n");
    const missingList = allIds.filter(id=>!player.owned.includes(id)).map(id=>{ const c=CHARACTERS[id]; return `> ❌ ${c.emoji} **${c.name}** \`[${c.grade}]\``; }).join("\n");
    return interaction.reply({ embeds:[new EmbedBuilder().setTitle(`📖 도감 (${player.owned.length}/${allIds.length})`).setColor(0x7C5CFC).setDescription(`**보유**\n${ownedList||"없음"}\n\n**미획득**\n${missingList||"없음"}`)] });
  }
  ...
  if (commandName === "출석") {
    ...
    player.crystals=(player.crystals||0)+cr; player.lastDaily=now; player.dailyStreak=(player.dailyStreak||0)+1;
    savePlayer(userId);
    return interaction.reply({ content:`✅ 출석 체크! +${cr}💎 (연속 ${player.dailyStreak}일)` });
  }

  if (commandName === "회복") {
    if ((player.potion||0) <= 0) return interaction.reply({ content:"❌ 회복약 없음!", ephemeral:true });
    const stats = getPlayerStats(player);
  ...
  if (commandName === "손가락") {
    ...
    return interaction.reply({ embeds:[new EmbedBuilder().setColor(0x8b0000).setTitle("👹 스쿠나 손가락").setDescription([`\`\`\`\n[${bar}]\n${fingers} / ${SUKUNA_FINGER_MAX} 개\n\`\`\``,`> **${bonus.label}**`,`> 🗡️ ATK +${bonus.atkBonus} · 🛡️ DEF +${bonus.defBonus} · 💚 HP +${bonus.hpBonus}`,fingers===0?`> 💡 전투에서 스쿠나를 처치하면 손가락 획득!`:"",(player.owned||[]).includes("sukuna")?`> 🔴 스쿠나 해금됨!`:""].filter(Boolean).join("\n"))] });
  }
  ...
  if (commandName === "결투") {
    ...
    const ch1=CHARACTERS[player.active], chT=CHARACTERS[players[target.id].active];
    const s1=getPlayerStats(player), s2=getPlayerStats(players[target.id]);
    const embed = new EmbedBuilder().setTitle("⚔️ PvP 결투 신청").setColor(0xF5C842)
      .setDescription([`${target}님, **${user.username}**님이 결투를 신청했습니다!`,`> 도전자: ${ch1.emoji} **${ch1.name}** [${ch1.grade}]  HP: ${s1.maxHp}  ATK: ${s1.atk}`,`> 상대방: ${chT.emoji} **${chT.name}** [${chT.grade}]  HP: ${s2.maxHp}  ATK: ${s2.atk}`].join("\n")).setFooter({text:"30초 내 수락/거절"});
  ...
  if (commandName === "파티초대") {
    ...
    await interaction.reply({ content:`${target}`, embeds:[new EmbedBuilder().setTitle("👥 파티 초대").setColor(0x4ade80).setDescription(`${target}님, **${user.username}**님의 파티에 초대받았습니다!`)], components:[buttons] });
  ...
  if (commandName === "레이드") {
    ...
    if (!RAID_BOSSES[bossId]) return interaction.reply({ content:`❌ 존재하지 않는 보스! 가능: heian_sukuna, mahoraga`, ephemeral:true });
    if (getRaidByUser(userId)) return interaction.reply({ content:"❌ 이미 레이드 중!", ephemeral:true });
    const party = getParty(userId); const members = party?[...party.members]:[userId];
    const boss = RAID_BOSSES[bossId]; const raidId = `${_raidIdSeq++}`;
    raidSessions[raidId] = {id:raidId,bossId,hp:boss.hp,enraged:false,members:[...members],adaptedSkills:[]};
    for (const uid of members) { const p=players[uid]; if (p&&(p.hp||0)<=0) { const s=getPlayerStats(p); p.hp=Math.floor(s.maxHp*0.5); savePlayer(uid); } }
    const introEmbed = new EmbedBuilder().setTitle(`🔥 레이드 시작: ${boss.name}`).setColor(boss.color)
      .setDescription([`\`\`\`ansi\n\u001b[1;31m╔══════════════════════════════════╗\n║  ⚔️  RAID BATTLE START!  ⚔️       ║\n╚══════════════════════════════════╝\n\`\`\``,`> *"${boss.lore}"*`,`> 💚 HP: **${boss.hp}**  🗡️ ATK: **${boss.atk}**  🛡️ DEF: **${boss.def}**`,`> 참여: ${members.length}명`].join("\n"));
  ...
  if (commandName === "코드") {
    ...
    if (CODES[code]) { player.crystals=(player.crystals||0)+(CODES[code].crystals||0); if(!player.usedCodes)player.usedCodes=[]; player.usedCodes.push(code); savePlayer(userId); return interaction.reply({ content:`✅ 코드 사용! +${CODES[code].crystals||0}💎` }); }
  ...
  if (commandName === "주구제작") {
    ...
    const mats = player.materials||{};
    for (const [mat,qty] of Object.entries(w.recipe)) { if ((mats[mat]||0)<qty) { return interaction.reply({ content:`❌ 재료 부족! ${MATERIALS[mat]?.emoji||""}**${mat}** ${mats[mat]||0}/${qty}`, ephemeral:true }); } }
    ...
    updateQuestProgress(player,"weapon_craft",1); savePlayer(userId);
    return interaction.reply({ embeds:[new EmbedBuilder().setTitle(`${w.emoji} ${w.name} 제작 완료!`).setColor(w.color).setDescription(`> **등급:** ${w.grade}\n> ATK+${w.atkBonus} DEF+${w.defBonus} HP+${w.hpBonus}\n> \`/장착\` 으로 장착!`)] });
  }
  ...
  if (commandName === "장착") {
    ...
    player.equippedWeapon = w.name; savePlayer(userId);
  ...
  if (commandName === "해제") {
    ...
    const w = getWeaponByName(player.equippedWeapon);
    player.equippedWeapon = null; savePlayer(userId);
  ...
  if (commandName === "도움말") {
    return interaction.reply({ embeds:[new EmbedBuilder().setColor(0xF5C842).setTitle("🔱 주술회전 RPG — 명령어 목록")
      .setDescription(["**⚔️ 전투 시스템**","> `/전투` — 일반 전투 (5% 확률 스쿠나!)","> `/컬링` — 웨이브 컬링","> `/사멸회유` — 포인트 게임","> `/결투 @유저` — PvP","","> **`/레이드 heian_sukuna`** — 헤이안 스쿠나","> **`/레이드 mahoraga`** — 마허라가라","","**🎭 캐릭터**","> `/활성` — 캐릭터 선택","> `/가챠 1|10` — 소환 (150/1350💎)","> `/도감` — 보유 캐릭터","> `/손가락` — 스쿠나 손가락","","**📋 기타**","> `/프로필` `/술식` `/출석` `/회복` `/퀘스트`","> `/재료` `/주구목록` `/주구제작` `/장착` `/해제`","> `/코가네가챠` `/코가네` `/코드`","","**⭐ 특수**","> ⚫ 흑섬: 10% → ×2.5 +50💎","> ✨ 치명타: 5% → ×1.5","> 👹 스쿠나: 처치 시 손가락 획득 → 1개로 해금!","> 🧪 회복약: 35~100% 드롭","> 🔄 마허라가라: 맞은 술식 다음 턴 면역!"].join("\n"))
      .setFooter({text:"🔱 주술회전 RPG | !프로필 로 GIF 프로필 확인!"})] });
  }
}
...
client.on("messageCreate", async (message) => {
  ...
  try {
    // ── !가입
    if (cmd === "가입") {
      const existed = !!players[userId];
      getPlayer(userId, message.author.username);
      savePlayer(userId);
      if (existed) return message.reply(`✅ 이미 가입되어 있습니다!\n> \`!프로필\` 로 내 정보 확인 | \`!도움말\` 로 명령어 확인`);
      return message.reply(`🎴 **${message.author.username}** 님, 주술회전 RPG에 오신 것을 환영합니다!\n> 기본 캐릭터 '이타도리 유지' 와 500💎, 회복약 3개 지급!\n> \`!도움말\` 로 명령어를 확인하세요!`);
    }

    // ── !탈퇴
    if (cmd === "탈퇴") {
      if (!players[userId]) return message.reply("❌ 가입되지 않은 사용자입니다.");
      delete battles[userId]; delete cullings[userId]; delete jujutsus[userId];
      const party = getParty(userId);
      if (party) { party.members=party.members.filter(id=>id!==userId); if(party.members.length===0) delete parties[party.id]; else if(party.leader===userId) party.leader=party.members[0]; }
      const pvp = getPvpSessionByUser(userId);
      if (pvp) { const sid=Object.keys(pvpSessions).find(k=>pvpSessions[k]===pvp); if(sid) delete pvpSessions[sid]; }
      const raid = getRaidByUser(userId);
      if (raid) { raid.members=raid.members.filter(id=>id!==userId); if(raid.members.length===0){const sid=Object.keys(raidSessions).find(k=>raidSessions[k]===raid);if(sid)delete raidSessions[sid];} }
      delete players[userId];
      await dbDelete(userId);
      if (saveQueue.has(userId)) clearTimeout(saveQueue.get(userId));
      savePending.delete(userId);
      return message.reply("🗑️ 탈퇴 처리 완료. 모든 데이터 삭제. 다시 시작하려면 `!가입` 해주세요.");
    }

    // ── !프로필 (GIF — 숫자 기반 상태창 UI)
    if (cmd === "프로필") {
      const loadingMsg = await message.reply("⏳ 프로필 카드 생성 중...");
      try {
        ...
        console.error("[!프로필] GIF 오류:", err.message);
        await loadingMsg.edit("❌ GIF 생성 실패. 텍스트 프로필을 표시합니다.").catch(()=>{});
    ...
    // ── !활성 / !캐릭터
    if (cmd === "활성" || cmd === "캐릭터") {
      if (player.owned.length === 0) return message.reply("❌ 보유 캐릭터 없음! `!가챠` 로 캐릭터를 뽑으세요.");
      return message.reply({ content:"🎭 **캐릭터를 선택하세요:**", components:[mkCharSelectMenu(player,"char_select")] });
    }
    ...
    if (cmd === "도감") {
      ...
      const ownedList = player.owned.map(id=>{
        ...
        const m=getMastery(player,id); const ri=GACHA_RARITY[c.grade]||GACHA_RARITY["3급"];
        return `> ${id===player.active?"▶️**[활성]**":"　"}${c.emoji} **${c.name}** \`[${c.grade}]\` ${ri.stars} 숙련\`${m}\``;
      }).join("\n");
      const missingList = allIds.filter(id=>!player.owned.includes(id)).map(id=>{ const c=CHARACTERS[id]; return `> ❌ ${c.emoji} **${c.name}** \`[${c.grade}]\``; }).join("\n");
      return message.reply({ embeds:[new EmbedBuilder().setTitle(`📖 도감 (${player.owned.length}/${allIds.length})`).setColor(0x7C5CFC).setDescription(`**보유**\n${ownedList||"없음"}\n\n**미획득**\n${missingList||"없음"}`).setFooter({text:"!활성 으로 캐릭터를 변경할 수 있습니다."})] });
    }

    // ── !전투
    if (cmd === "전투") {
      if (battles[userId]) return message.reply("❌ 이미 전투 중!");
      const eBase = Math.random()<0.05 ? ENEMIES.find(e=>e.id==="e_sukuna") : ENEMIES[Math.floor(Math.random()*ENEMIES.length)];
      const enemy = {...eBase, currentHp:eBase.hp, statusEffects:[]};
      battles[userId] = {enemy};
      const stats = getPlayerStats(player);
      const embed = new EmbedBuilder().setTitle(eBase.id==="e_sukuna"?"🔴 료멘 스쿠나 출현!":"⚔️ 전투 시작!").setColor(eBase.id==="e_sukuna"?0x8b0000:0xff0000)
    ...
    if (cmd === "가챠" || cmd === "가챠10") {
      ...
      await new Promise(r=>setTimeout(r,1500)); await loadingMsg.edit({ embeds:[gachaLoadingEmbed(2)] });
      await new Promise(r=>setTimeout(r,1500)); await loadingMsg.edit({ embeds:[gachaLoadingEmbed(3)] });
      await new Promise(r=>setTimeout(r,1500));
      if (count === 1) {
        const result = rollGacha(1)[0]; const isNew = !player.owned.includes(result);
        if (isNew) { player.owned.push(result); if (!player.mastery[result]) player.mastery[result]=0; } else player.crystals += 50;
        await loadingMsg.edit({ embeds:[gachaRevealEmbed(CHARACTERS[result].grade), gachaResultEmbed(result,isNew,player)] });
      } else {
        const results = rollGacha(10); const dupCrystals=results.filter(id=>player.owned.includes(id)).length*50; const newOnes=results.filter(id=>!player.owned.includes(id));
        for (const id of newOnes) { player.owned.push(id); if (!player.mastery[id]) player.mastery[id]=0; }
        player.crystals += dupCrystals;
    ...
    if (cmd === "출석") {
      ...
      player.crystals=(player.crystals||0)+cr; player.lastDaily=now; player.dailyStreak=(player.dailyStreak||0)+1;
      savePlayer(userId);
      return message.reply(`✅ 출석 체크! +${cr}💎 (연속 ${player.dailyStreak}일)`);
    }

    // ── !회복
    if (cmd === "회복") {
      if ((player.potion||0) <= 0) return message.reply("❌ 회복약 없음! 전투에서 획득하세요.");
    ...
    if (cmd === "구매") {
      ...
      if ((player.crystals||0)<cost) return message.reply(`💎 크리스탈 부족! (필요: ${cost})`);
      player.crystals -= cost; player.potion = (player.potion||0)+amount;
      savePlayer(userId);
    ...
    // ── !코가네가챠
    if (cmd === "코가네가챠") {
      if ((player.crystals||0)<200) return message.reply("💎 크리스탈 부족! (필요: 200)");
      player.crystals -= 200; player.koganeGachaCount = (player.koganeGachaCount||0)+1;
      const loadingMsg = await message.reply({ embeds:[koganeLoadingEmbed(1)] });
      await new Promise(r=>setTimeout(r,1800)); await loadingMsg.edit({ embeds:[koganeLoadingEmbed(2)] });
      await new Promise(r=>setTimeout(r,1800)); await loadingMsg.edit({ embeds:[koganeLoadingEmbed(3)] });
      await new Promise(r=>setTimeout(r,1500));
      const grade = rollKogane();
      const gradeOrder = ["3급","2급","1급","특급","전설"];
      const isUpgrade = !player.kogane||gradeOrder.indexOf(grade)>gradeOrder.indexOf(player.kogane.grade);
    ...
    if (cmd === "주구제작") {
      ...
      const mats = player.materials||{};
      for (const [mat,qty] of Object.entries(w.recipe)) { if ((mats[mat]||0)<qty) return message.reply(`❌ 재료 부족! ${MATERIALS[mat]?.emoji||""}**${mat}** ${mats[mat]||0}/${qty}`); }
      for (const [mat,qty] of Object.entries(w.recipe)) mats[mat] -= qty;
      if (!player.craftedWeapons) player.craftedWeapons=[];
      player.craftedWeapons.push(w.id);
      updateQuestProgress(player,"weapon_craft",1); savePlayer(userId);
      return message.reply(`✅ **${w.emoji} ${w.name}** 제작 완료! \`!장착 ${w.name}\` 로 장착`);
    }

    // ── !장착
    if (cmd === "장착") {
      const weaponName = args.slice(1).join(" "); const w = getWeaponByName(weaponName);
    ...
    if (cmd === "해제") {
      ...
      const w = getWeaponByName(player.equippedWeapon); player.equippedWeapon=null; savePlayer(userId);
    ...
    // ── !퀘보상
    if (cmd === "퀘보상") {
      const type=args[1]; const idx=parseInt(args[2])-1;
      if (type!=="일"&&type!=="주") return message.reply("❌ !퀘보상 일 [번호] 또는 !퀘보상 주 [번호]");
      initQuests(player);
      const isWeekly = type==="주"; const list=isWeekly?player.quests.weekly:player.quests.daily;
    ...
    if (cmd === "레이드") {
      ...
      const party=getParty(userId); const members=party?[...party.members]:[userId];
      const boss=RAID_BOSSES[bossId]; const raidId=`raid_${Date.now()}`;
      raidSessions[raidId] = {id:raidId,bossId,hp:boss.hp,enraged:false,members:members,adaptedSkills:[]};
      for (const uid of members) { const p=players[uid]; if(p&&(p.hp||0)<=0){const s=getPlayerStats(p);p.hp=Math.floor(s.maxHp*0.5);savePlayer(uid);} }
      return message.reply({ embeds:[raidEmbed(raidSessions[raidId])], components:[mkRaidButtons(player)] });
    }

    // ── !코드
    if (cmd === "코드") {
      const code = args[1]?.toLowerCase(); if (!code) return message.reply("!코드 [코드명]");
      if ((player.usedCodes||[]).includes(code)) return message.reply("❌ 이미 사용한 코드!");
      if (CODES[code]) { player.crystals=(player.crystals||0)+(CODES[code].crystals||0); if(!player.usedCodes)player.usedCodes=[]; player.usedCodes.push(code); savePlayer(userId); return message.reply(`✅ 코드 사용! +${CODES[code].crystals||0}💎`); }
    ...
    if (cmd === "파티생성") {
      ...
      const partyId=`${_partyIdSeq++}`; parties[partyId]={id:partyId,leader:userId,members:[userId],bestWave:0};
      return message.reply(`✅ 파티 생성! (1/4명)`);
    }

    // ── !파티초대
    if (cmd === "파티초대") {
      const target = message.mentions.users.first(); if (!target) return message.reply("❌ !파티초대 @유저");
    ...
    if (cmd === "파티컬링") {
      ...
      const firstEnemy=pickCullingEnemy(1); cullings[party.id]={wave:1,kills:0,totalXp:0,totalCrystals:0,currentEnemy:firstEnemy,enemyHp:firstEnemy.hp};
      return message.reply({ embeds:[partyCullingEmbed(party,cullings[party.id])], components:[mkCullingButtons(player)] });
    }

    // ── 개발자 명령어
    if (cmd === "개발자패널" && isDev(userId)) return message.reply({ embeds:[devPanelEmbed()] });

    if (cmd === "쿨다운초기화" && isDev(userId)) {
      player.skillCooldown=0; player.reverseCooldown=0; savePlayer(userId);
    ...
    if (cmd === "아이템지급" && isDev(userId)) {
      ...
      const targetPlayer = getPlayer(targetId, targetUser?.username||"개발자지급");
      const item = args[targetUser?2:1]; const amount = parseInt(args[targetUser?3:2])||1;
      if (!item) return message.reply("❌ !아이템지급 [@유저] [크리스탈|회복약|손가락|재료명] [수량]");
      if (item==="크리스탈") targetPlayer.crystals=(targetPlayer.crystals||0)+amount;
      else if (item==="회복약") targetPlayer.potion=(targetPlayer.potion||0)+amount;
      else if (item==="손가락") targetPlayer.sukunaFingers=Math.min(SUKUNA_FINGER_MAX,(targetPlayer.sukunaFingers||0)+amount);
      else if (MATERIALS[item]) { if(!targetPlayer.materials)targetPlayer.materials={}; targetPlayer.materials[item]=(targetPlayer.materials[item]||0)+amount; }
      else return message.reply("❌ 아이템: 크리스탈, 회복약, 손가락, 또는 재료명");
      savePlayer(targetId);
      return message.reply(`✅ **${targetPlayer.name}** 에게 ${item} +${amount} 지급!`);
    }
    ...
    if (cmd === "플레이어정보" && isDev(userId)) {
      ...
      const p = players[target.id]; if (!p) return message.reply("❌ 플레이어 정보 없음");
      const stats = getPlayerStats(p);
      const matSummary=Object.entries(p.materials||{}).filter(([,q])=>q>0).map(([id,q])=>`${MATERIALS[id]?.emoji||""}${q}`).join(" ")||"없음";
      return message.reply(`📊 **${p.name||"??"}**\n💎 ${p.crystals||0} | XP ${p.xp||0} | LV.${getLevel(p.xp||0)}\n🎭 ${CHARACTERS[p.active]?.name||p.active} | ATK ${stats.atk} | DEF ${stats.def} | HP ${p.hp||0}/${stats.maxHp}\n⚔️ ${p.wins||0}승 ${p.losses||0}패 | PvP ${p.pvpWins||0}승\n🧪 회복약: ${p.potion||0}개 | 👹 손가락: ${p.sukunaFingers||0}개\n⚡ 치명타: ${p.crit||5}% | 장착: ${p.equippedWeapon||"없음"}\n📦 재료: ${matSummary}`);
    }
    ...
    if (cmd === "도움말") {
      return message.reply([
        ...
        "⚔️ `!컬링` — 컬링 게임",
        "⚔️ `!사멸회유` — 포인트 게임",
        "⚔️ `!레이드 [heian_sukuna|mahoraga]` — 레이드","",
        "⚔️ `!재료` `!주구목록` `!주구제작` `!장착` `!해제`","",
        "📋 `!퀘스트` `!퀘보상 일/주 [번호]`",
        "📋 `!출석` `!회복` `!구매` `!코드` `!코가네가챠`",
        "📋 `!파티생성` `!파티초대` `!파티나가기` `!파티컬링`","",
        "⭐ 흑섬 10% → ×2.5 | 치명타 5% → ×1.5",
        "👹 스쿠나 처치 시 손가락 획득 → 1개로 캐릭터 해금!",
        "🧪 회복약 전투 드랍 35~100% | 🔄 마허라가라: 술식 적응",
    ...
    console.error(`[messageCreate] cmd=${cmd} err:`, err.message);
    message.reply("❌ 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.").catch(()=>{});
  }
});
